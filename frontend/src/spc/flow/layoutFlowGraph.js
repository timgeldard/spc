/**
 * Layered layout for a DAG of material process nodes.
 * Uses Sugiyama-style: topological sort → layer assignment → position assignment.
 *
 * @param {Array<{id: string}>} nodes
 * @param {Array<{source: string, target: string}>} edges
 * @returns {Array<{id: string, position: {x: number, y: number}}>}
 */
export function layoutFlowGraph(nodes, edges) {
  ...
  while (queue.length > 0) {
    ...
    for (const succ of successors.get(curr)) {
      ...
      if (!layer.has(succ) || layer.get(succ) < proposed) {
