...
export function useLockedLimits(materialId, micId, plantId, chartType) {
  const [lockedLimits, setLockedLimits] = useState(null)
  ...
  const fetchLimits = useCallback(() => {
    ...
    return fetch(`/api/spc/locked-limits?${params}`)
      ...
      .then(d => setLockedLimits(d.locked_limits ?? null))
  ...
  const saveLimits = useCallback(async (limitsObj) => {
