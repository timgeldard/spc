...
export default function CorrelationMatrix({ pairs, mics, onCellClick }) {
  const { xLabels, yLabels, bgData, bubbleData } = useMemo(() => {
    ...
    for (const p of pairs) {
      pairMap.set(`${p.mic_a}__${p.mic_b}`, p)
      pairMap.set(`${p.mic_b}__${p.mic_a}`, { ...p, pearson_r: p.pearson_r })
    }

    const bg     = []   // uniform background cells
    const bubble = []   // circles with size = |r|
    ...
    for (let yi = 0; yi < micIds.length; yi++) {
      for (let xi = 0; xi < micIds.length; xi++) {
        ...
        const item = [xi, yi, r, micIds[xi], micIds[yi], n]
    ...
    return {
      ...
      series: [
        ...
        },
        {
          ...
          type: 'scatter',
          data: bubbleData,
          symbolSize: (val) => Math.max(3, Math.abs(val[2]) * MAX_BUBBLE),
          itemStyle: {
            color: (params) => rToColor(params.value[2]),
